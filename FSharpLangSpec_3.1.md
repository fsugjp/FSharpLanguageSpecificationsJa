...
インターフェイス型は一連のオブジェクト型をカプセル化できます：
...
この例にある`IEncoding`は`Encode`と`Decode`というオブジェクト型を
両方含むようなインターフェイス型です。
...
厳密なオブジェクト指向の型階層を設計することにあまり時間をかけずに、
ラピッドプロトタイピングを行うような場合に役立ちます。
...
行う事ができるようになります。
クエリ式ではF#における統合言語クエリ(LINQ: Language-Integrated Query)が
サポートされています。
また、さらに複雑なクエリを作成できるように、いくつかのクエリ演算子も用意されています。
